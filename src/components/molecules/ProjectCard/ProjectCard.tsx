...
export const ProjectCard: React.FC<ProjectCardProps> = ({ 
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    if (variant === 'flip' && (e.key === 'Enter' || e.key === ' ')) {
      e.preventDefault()
      setIsFlipped(prev => !prev)
    }
  }
  ...
  if (variant === 'flip') {
    return (
      <article 
        ...
        tabIndex={0}
        role="button"
        aria-label={`${title} - Click to ${isFlipped ? 'hide' : 'view'} details`}
        aria-pressed={isFlipped}
      >
        <div className={styles.flipCardInner}>
          {/* Back - visible by default (shows title + hint) */}
          <div className={styles.flipCardBack}>
            <div className={styles.flipCircles}>
            ...
            <div className={styles.flipBackContent}>
              <strong className={styles.flipBackTitle}>
                {title.split('Munguti').length > 1 ? (
                  <>
                    {title.split('Munguti').map((part, i, arr) => (
                      <span key={i}>
                        {part}
                        {i < arr.length - 1 && <span className="brand-font">Munguti</span>}
                      </span>
                    ))}
                  </>
                ) : (
              ...
              <span className={styles.flipHint}>Hover to explore</span>
            </div>
          </div>

          {/* Front - revealed on hover (shows details) */}
          <div className={styles.flipCardFront}>
            <div className={styles.flipFrontContent}>
              <svg className={styles.flipIcon} stroke="currentColor" viewBox="0 0 50 50" height="50px" width="50px" fill="currentColor">
              ...
              <h4 className={styles.flipTitle}>{title}</h4>
              {description && <p className={styles.flipDesc}>{description}</p>}
              {badges.length > 0 && (
                <div className={styles.flipBadges}>
                  {badges.map((b, i) => (
                    <span key={i} className={styles.flipBadge}>{b}</span>
                  ))}
                </div>
              )}
              {href && (
                <a 
                  className={styles.flipCta} 
                  href={href}
    ...
    <article className={styles.card}>
      ...
      <div className={styles.content}>
        ...
        <div className={styles.footer}>
          <a className={styles.cta} href={href || '#'}>View project</a>
        </div>
      </div>
    </article>
  )
